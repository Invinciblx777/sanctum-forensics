...
| 6 | What happens if backup verification fails? | The benchmark write refuses. It re-runs the backup verification itself straight before writing, and writes only if `sufficient_for_restoring_the_modified_region` is true. | `scripts/media_benchmark.py:verify_backup`, `write_image`; `tests/scripts/test_media_benchmark_boundary.py`; `qa.md` §25 | SYNTHETICALLY VALIDATED. The benchmark write re-verifies its own backup; a real whole-drive erase, since 2026-09-25, needs a backup image the server hashed and sized, bound by size, mtime, ctime and inode and re-checked by the helper (`tests/test_authorization_binding.py`). Neither proves the image is a copy of the device. Restoration HARDWARE-UNVERIFIED |
