...
**Date:** 2026-09-24 · **Code frozen at:** `8e1777d` (this card and the
consistency fixes that came with it are documentation only) · Companion to
...
|---|---|---|
| **PHYSICALLY VALIDATED** | Ran on a real device, recorded in `hardware.md` | One Toshiba TransMemory USB stick (7.76 GB): three Phase A overwrite Clear runs (the third clean, 2026-09-05), a power-cycle re-verification, three Phase B recovery passes, the mounted-device refusal (2026-09-23) |
...
| "Is that real hardware?" | Physical only for the overwrite Clear and the recovery passes on one USB stick and the mounted-device refusal. Everything else on screen is synthetic or simulation, and it is labelled. | Overview, LIMITATIONS: NOT PHYSICALLY VALIDATED column; `hardware.md` run table |
...
| 2 | What prevents `/dev/sda` changing to another device? | Straight before execution the engine re-reads the device at that path. If the serial or the `/dev/disk/by-id` link differs, it raises `DeviceVanished` and writes nothing. | `core/erase/drive.py:_reread_serial`; `tests/erase/test_drive_loopback.py::test_serial_swap_between_confirmation_and_execution_is_caught` | Code present. The test that proves it needs root and a loop device, so it is **skipped** in the unprivileged suite. NOT CURRENTLY PROVEN in the recorded suite run |
...
| 4 | What happens if the serial does not match? | `ConfirmationMismatch` with the remediation "Nothing has been modified". The erase does not start. | `core/device/guard.py:assert_serial_confirmed`; `tests/device/test_guard.py`, `tests/helper/test_daemon.py`; Sanitize screen: erase button disabled until the serial matches (`browser-2026-09-24/08`) | SYNTHETICALLY VALIDATED; UI on fixture devices |
...
| 6 | What happens if backup verification fails? | The benchmark write refuses. It re-runs the backup verification itself straight before writing, and writes only if `sufficient_for_restoring_the_modified_region` is true. | `scripts/media_benchmark.py:verify_backup`, `write_image`; `tests/scripts/test_media_benchmark_boundary.py`; `qa.md` §25 | SYNTHETICALLY VALIDATED. Scope is the benchmark write only; whole-drive sanitization has no backup gate by design. Restoration HARDWARE-UNVERIFIED |
...
| 11 | What requires human approval before execution? | Whole-drive erase: turning dry run off and typing the serial. Benchmark write: a recorded methodology decision, identity reconfirmation, manual unmount, a verified backup, a reviewed plan, and running `write` with the acknowledgement flag. | `api/routes/models.py` (`dry_run` default true); `core/workflow.py` (HUMAN_APPROVAL_REQUIRED); `physical-benchmark-checklist.md` | SYNTHETICALLY VALIDATED; UI on fixture devices (`browser-2026-09-24/08`) |
...
| Human approval gate: dry run default plus typed serial | Q4, Q11; `browser-2026-09-24/08` | SYNTHETICALLY VALIDATED; UI on fixtures |
